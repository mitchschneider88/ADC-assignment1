...
double Biquad::processAudioSample(double xn)
{
    ...
    /*else if (parameters.biquadCalcType == biquadAlgorithm::schneiderWDF) // this doesn't work yet
    {
        
        double beta0 = coeffArray[a0];
        
        double beta1 = coeffArray[a0] - coeffArray[a2] - ((coeffArray[a0] - coeffArray[a1] - coeffArray[a2]) / 2);
        
        double beta2 = (coeffArray[a0] - coeffArray[a1] - coeffArray[a2]) / 2;
        
        double gamma1 = coeffArray[b1] + ((1 - coeffArray[b1] - coeffArray[b2]) / 2);
        
        double gamma2 = (1 - coeffArray[b1] - coeffArray[b2]) / 2;
        
        double yn = (beta0 * xn) + stateArray[x_z1] + stateArray[y_z1];
        
        checkFloatUnderflow(yn);
        
        stateArray[x_z1] = ((xn * beta1) - (yn * gamma1)) + stateArray[y_z1];
        
        stateArray[y_z1] = ((xn * beta2) - (yn * gamma2)) - stateArray[x_z1];
...
bool AudioFilter::calculateFilterCoeffs()
{
    ...
    else if (algorithm == filterAlgorithm::schneiderHiShelf)
    {
        
        // https://www.earlevel.com/main/2011/01/02/biquad-formulas/
        
        double theta_c_normalized = tan(2.0*kPi*fc / sampleRate); // normalizing the frequency
        
        DBG("theta normalized is" + std::to_string(theta_c_normalized));
        
        double A = sqrt(pow(10, (boostCut_dB / 10)));
        
        DBG("A is" + std::to_string(A));

        double norm = 1 / (1 + 1/Q * theta_c_normalized + theta_c_normalized * theta_c_normalized); // the "b0" coefficient the rest of the coefficients are normalized to
        
        coeffArray[a0] = (A + sqrt(A)/Q * theta_c_normalized + theta_c_normalized * theta_c_normalized) * norm;
        
        DBG("hishelf a0 is" + std::to_string(coeffArray[a0]));
        
        coeffArray[a1] = 2 * (theta_c_normalized * theta_c_normalized - A) * norm;
        
        DBG("hishelf a1 is" + std::to_string(coeffArray[a1]));
        
        coeffArray[a2] = (A - sqrt(A)/Q * theta_c_normalized + theta_c_normalized * theta_c_normalized) * norm;
        
        DBG("hishelf a2 is" + std::to_string(coeffArray[a2]));
        
        coeffArray[b1] = 2 * (theta_c_normalized * theta_c_normalized - 1) * norm;
        
        DBG("hishelf b1 is" + std::to_string(coeffArray[b1]));
        
        coeffArray[b2] = (1 - 1/Q * theta_c_normalized + theta_c_normalized * theta_c_normalized) * norm;
        
        DBG("hishelf b2 is" + std::to_string(coeffArray[b2]));
        // --- update on calculator
        biquad.setCoefficients(coeffArray);
    ...
    else if (algorithm == filterAlgorithm::schneiderLoShelf)
    {
        
        // https://www.earlevel.com/main/2011/01/02/biquad-formulas/
        
        double theta_c_normalized = tan(2.0*kPi*fc / sampleRate);
        ...
        double norm = 1 / (1 + 1/Q * theta_c_normalized + theta_c_normalized * theta_c_normalized);
        
        coeffArray[a0] = (1 + sqrt(A)/Q * theta_c_normalized + A * theta_c_normalized * theta_c_normalized) * norm;
        
        DBG("loshelf a0 is" + std::to_string(coeffArray[a0]));
        
        coeffArray[a1] = 2 * (A * theta_c_normalized * theta_c_normalized - 1) * norm;
        
        DBG("loshelf a1 is" + std::to_string(coeffArray[a1]));
        
        coeffArray[a2] = (1 - sqrt(A)/Q * theta_c_normalized + A * theta_c_normalized * theta_c_normalized) * norm;
        
        DBG("loshelf a2 is" + std::to_string(coeffArray[a2]));
        
        coeffArray[b1] = 2 * (theta_c_normalized * theta_c_normalized - 1) * norm;
        
        DBG("loshelf b1 is" + std::to_string(coeffArray[b1]));
        
        coeffArray[b2] = (1 - 1/Q * theta_c_normalized + theta_c_normalized * theta_c_normalized) * norm;
        
        DBG("loshelf b2 is" + std::to_string(coeffArray[b2]));
        // --- update on calculator
        biquad.setCoefficients(coeffArray);
